...
import { createProPixCheckoutSession, stripePixConfigured } from "@/lib/stripe";
...
export async function POST(request: Request) {
  ...
  if (!stripePixConfigured()) {
    return Response.json({ ok: false, error: "pix_not_configured" }, { status: 503, headers: NO_STORE_HEADERS });
  }
  ...
  // Uma assinatura Stripe existente deve ser resolvida pelo Portal, não
  // contornada por uma compra Pix paralela.
  ...
  try {
    const checkout = await createProPixCheckoutSession({
      userId: session.user.id,
      email: session.user.email,
    });

    if (!checkout.url) throw new Error("Stripe Pix Checkout did not return a URL");

    return Response.json({ ok: true, url: checkout.url }, { headers: NO_STORE_HEADERS });
  } catch (error) {
    console.error("stripe_pix_checkout_error", error instanceof Error ? error.message : "unknown_error");
    return Response.json({ ok: false, error: "pix_checkout_failed" }, { status: 502, headers: NO_STORE_HEADERS });
  }
}
