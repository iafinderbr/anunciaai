          ...
          <div className="mx-auto max-w-6xl">
            <div className="flex flex-col gap-5 rounded-3xl border border-line bg-white p-5 shadow-card sm:flex-row sm:items-center sm:justify-between sm:p-6">
              <div className="flex min-w-0 items-center gap-4">
                <span aria-hidden="true" className="grid size-12 shrink-0 place-items-center rounded-2xl bg-ink text-lg font-semibold text-white">
                  {initial}
                </span>
                <div className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-brand-600">Minha conta</p>
                  <h1 className="mt-1 truncate text-2xl font-semibold tracking-tight text-ink">Olá, {session.user.name}</h1>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <Link href="/conta/historico" className="group rounded-2xl border border-brand-200 bg-brand-50/55 p-5 shadow-card transition-colors hover:border-brand-400">
              ...
              <Link href="/conta/produtos" className="group rounded-2xl border border-brand-200 bg-white p-5 shadow-card transition-colors hover:border-brand-400">
                <p className="text-xs font-semibold uppercase tracking-[0.1em] text-brand-600">Produtos</p>
                <div className="mt-2 flex items-end justify-between gap-3">
                  <span className="text-3xl font-semibold tracking-tight text-ink">{productsCount}</span>
                  <span className="text-sm font-semibold text-brand-700">Biblioteca →</span>
                </div>
                <p className="mt-2 text-xs leading-5 text-muted">Dados de produtos escolhidos para reutilização.</p>
              </Link>

              <Link href="/conta/plano" className="group rounded-2xl border border-line bg-white p-5 shadow-card transition-colors hover:border-brand-300">
              ...
              <Link href="/ferramentas" className="group rounded-2xl border border-line bg-white p-5 shadow-card transition-colors hover:border-brand-300">
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              <section aria-labelledby="recentes-titulo" className="rounded-3xl border border-line bg-white p-5 shadow-card sm:p-6">
                <div className="flex items-center justify-between gap-4">
                  ...
                  {savedCount > 0 ? (
                    <Link href="/conta/historico" className="text-sm font-semibold text-brand-700 hover:text-brand-800">Ver histórico →</Link>
                  ) : null}
                </div>
                ...
                {recentItems.length > 0 ? (
                  <div className="mt-5 divide-y divide-line overflow-hidden rounded-2xl border border-line">
                    {recentItems.map((item) => (
                      <Link key={item.id} href="/conta/historico" className="group flex items-center justify-between gap-4 bg-white px-4 py-4 transition-colors hover:bg-canvas">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="mt-5 rounded-2xl border border-dashed border-line-strong bg-canvas px-5 py-8 text-center">
                    ...
                    <Link href="/#ferramenta" className="mt-4 inline-flex rounded-xl bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-600">Criar anúncio</Link>
                  </div>
                )}
              </section>
              ...
              <aside className="space-y-4">
                <div className="rounded-3xl border border-line bg-white p-5 shadow-card">
                  ...
                  <div className="mt-4 grid gap-2">
                    <Link href="/#ferramenta" className="flex items-center justify-between rounded-xl bg-ink px-4 py-3 text-sm font-semibold text-white hover:bg-brand-600">Criar anúncio <span aria-hidden="true">→</span></Link>
                    <Link href="/conta/produtos" className="flex items-center justify-between rounded-xl border border-line-strong bg-canvas px-4 py-3 text-sm font-semibold text-ink hover:border-brand-300 hover:text-brand-700">Produtos salvos <span aria-hidden="true">→</span></Link>
                    <Link href="/ferramentas" className="flex items-center justify-between rounded-xl border border-line-strong bg-canvas px-4 py-3 text-sm font-semibold text-ink hover:border-brand-300 hover:text-brand-700">Escolher gerador <span aria-hidden="true">→</span></Link>
                    <Link href="/guias" className="flex items-center justify-between rounded-xl border border-line-strong bg-canvas px-4 py-3 text-sm font-semibold text-ink hover:border-brand-300 hover:text-brand-700">Abrir guias <span aria-hidden="true">→</span></Link>
                  </div>
                </div>

                <Link href="/conta/produtos" className="block rounded-3xl border border-brand-200 bg-brand-50/45 p-5 shadow-card transition-colors hover:border-brand-400">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-ink">Produtos salvos</p>
                    <span className="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-brand-700">{productsCount}</span>
                  </div>
                  <p className="mt-2 text-xs leading-5 text-muted">Sua biblioteca privada já está pronta para receber os produtos que você escolher guardar.</p>
                  <p className="mt-3 text-xs font-semibold text-brand-700">Abrir biblioteca →</p>
                </Link>
              </aside>
            </div>
          </div>
