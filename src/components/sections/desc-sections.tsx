    ...
    <section id="como-funciona" aria-labelledby="desc-como-funciona-titulo" className="border-y border-line bg-white">
      <div className="container-page py-14 sm:py-20">
        <div className="max-w-2xl">
          ...
          <h2 id="desc-como-funciona-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
            Das características à descrição publicada em 3 passos
          </h2>
          <p className="mt-3 text-[15px] text-muted">
            Você não precisa ser redator. Precisa apenas saber o que o produto é e para quem ele serve.
          </p>
        </div>

        <ol className="mt-10 grid gap-4 md:grid-cols-3">
          {DESC_STEPS.map((step, index) => (
    ...
    <section aria-labelledby="desc-anatomia-titulo" className="container-page py-14 sm:py-20">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,420px)_1fr] lg:gap-16">
        <div>
          ...
          <h2 id="desc-anatomia-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
            O que uma boa descrição de produto precisa ter
          </h2>
          <p className="mt-3 text-[15px] text-muted">
            Uma descrição que vende não é um parágrafo cheio de elogios. É uma sequência de blocos que responde às
            dúvidas do comprador antes mesmo de ele perguntar.
          </p>

          <ol className="mt-6 space-y-4">
            {DESC_ANATOMY.map((item, index) => (
        ...
        <div className="lg:pt-10">
          <div className="rounded-2xl border border-line bg-white p-5 shadow-card sm:p-6">
            ...
            <h3 className="mt-2 text-lg font-semibold tracking-tight">Característica vira benefício</h3>
            <p className="mt-1.5 text-sm text-muted">
              O comprador não quer saber que a bateria tem 40 horas. Ele quer saber que vai usar a semana inteira sem
              recarregar.
            </p>

            <ul className="mt-5 divide-y divide-line overflow-hidden rounded-xl border border-line">
              {DESC_BENEFIT_EXAMPLES.map((item) => (
                <li key={item.feature} className="bg-canvas px-4 py-3">
                  ...
                  <p className="mt-2 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-brand-600">
                    <span aria-hidden="true">↓</span> Benefício
                  </p>
                  <p className="mt-0.5 text-sm font-medium text-ink">{item.benefit}</p>
                </li>
              ))}
            </ul>
          </div>
    ...
    <section id="recursos" aria-labelledby="desc-recursos-titulo" className="border-y border-line bg-white">
      <div className="container-page py-14 sm:py-20">
        <div className="max-w-2xl">
          ...
          <h2 id="desc-recursos-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
            Uma descrição completa, gerada de uma vez
          </h2>
          <p className="mt-3 text-[15px] text-muted">
            Você informa o produto uma única vez e recebe o texto pronto, com a estrutura que converte.
          </p>
        </div>

        <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {DESC_FEATURES.map((feature) => (
  ...
  return (
    <section aria-labelledby="desc-erros-titulo" className="container-page py-14 sm:py-20">
      <div className="max-w-2xl">
        ...
        <h2 id="desc-erros-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
          Erros comuns em descrições de produtos
        </h2>
        <p className="mt-3 text-[15px] text-muted">
      ...
      <div className="mt-10">
        <h3 className="text-base font-semibold">Na prática: descrição ruim × descrição melhor</h3>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-rose-200 bg-rose-50/40 p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-rose-600">Descrição fraca</p>
            <p className="mt-3 text-sm leading-relaxed text-rose-900/70">{DESC_COMPARISON.bad}</p>
          </div>
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-emerald-700">Descrição melhor</p>
            <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-emerald-900/80">
              {DESC_COMPARISON.good}
            </p>
          </div>
        </div>
      </div>
...
export function DescChannels() {
  return (
    <section aria-labelledby="desc-canais-titulo" className="border-y border-line bg-white">
      <div className="container-page py-14 sm:py-20">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-600">Adaptação</p>
          <h2 id="desc-canais-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
            A mesma descrição não serve para todo canal
          </h2>
          <p className="mt-3 text-[15px] text-muted">
            Loja virtual, marketplace e redes sociais têm ritmos diferentes. O texto precisa acompanhar.
          </p>
        </div>

        <ul className="mt-10 grid gap-4 md:grid-cols-3">
          {DESC_CHANNELS.map((channel) => (
...
export function DescFaq() {
  return (
    <section id="duvidas" aria-labelledby="desc-faq-titulo" className="border-t border-line bg-white">
      <div className="container-page py-14 sm:py-20">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,340px)_1fr] lg:gap-16">
          <div>
            ...
            <h2 id="desc-faq-titulo" className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
              Perguntas sobre descrições de produto
            </h2>
            <p className="mt-3 text-[15px] text-muted">
              O que os lojistas mais perguntam antes de escrever a primeira descrição com IA.
            </p>
          </div>
