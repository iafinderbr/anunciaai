...
export function GeneratorTool({
  ...
  return (
    <div ref={anchorRef} className="scroll-mt-24">
      {status === "form" ? (
        <form onSubmit={handleSubmit} noValidate className="overflow-hidden rounded-2xl border border-line bg-white shadow-lift">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-line bg-canvas/55 px-5 py-5 sm:px-7 sm:py-6">
            <div className="max-w-xl">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-brand-600">Gerador</p>
              <h3 className="mt-1.5 text-xl font-semibold tracking-tight text-ink sm:text-2xl">{title}</h3>
            ...
            <button
              ...
              className="rounded-lg border border-line-strong bg-white px-3 py-2 text-xs font-semibold text-ink-soft transition-colors hover:border-brand-300 hover:text-brand-700"
            >
              Usar exemplo
            </button>
          </div>
          ...
          <div className="px-5 pb-6 pt-1 sm:px-7 sm:pb-7">
            ...
            {lockedChannel ? (
              ...
              <fieldset className="mt-7 border-t border-line pt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {CHANNELS.map((channel) => {
                    ...
                    return (
                      <label
                        key={channel.value}
                        className={`cursor-pointer rounded-lg border px-3.5 py-2.5 text-sm font-medium transition-colors focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-brand-500 ${
                          active
                ...
                <p className="mt-2 text-xs leading-5 text-muted">{CHANNELS.find((channel) => channel.value === input.channel)?.hint}</p>
              </fieldset>
            )}
                  ...
                  return (
                    <label
                      ...
                      className={`cursor-pointer rounded-xl border px-3.5 py-3 transition-colors focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-brand-500 ${
                        active ? "border-ink bg-ink" : "border-line-strong bg-white hover:border-brand-200 hover:bg-canvas"
                      }`}
                    >
                      <input
                        type="radio"
                      ...
                      <span className={`mt-0.5 block text-xs ${active ? "text-white/60" : "text-muted"}`}>{tone.hint}</span>
                    </label>
                  );
                })}
              </div>
            ...
            <div className="mt-8 border-t border-line pt-6">
              <button type="submit" className="group flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-6 py-3.5 text-sm font-semibold text-white shadow-card transition-all hover:-translate-y-0.5 hover:bg-brand-600 hover:shadow-lift">
                Gerar anúncio
                <span aria-hidden="true" className="transition-transform group-hover:translate-x-0.5">→</span>
              </button>
              <p className="mt-3 text-center text-xs leading-5 text-muted">Grátis para começar. O resultado aparece nesta mesma página.</p>
            </div>
          </div>
        </form>
      ) : null}
      ...
      {status === "loading" ? (
        <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-lift" role="status" aria-live="polite">
          <div className="relative h-1 w-full overflow-hidden bg-line loading-bar" />
          <div className="p-6 sm:p-8">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-brand-600">Gerando</p>
            <h3 className="mt-2 text-xl font-semibold tracking-tight text-ink">Preparando seu anúncio…</h3>
            <p className="mt-1.5 text-sm leading-6 text-muted">Estamos organizando título, descrição, benefícios e SEO para {input.productName || "o seu produto"}.</p>
